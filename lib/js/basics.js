...
function scrollNShow(appear, nodes){
	...
	appear = appear? appear : function(node, invisibleClass, visibleClass){
		if(hasClass(node, "timeline-based")){
			setTimeout(visible, node.getAttribute("data-playtime"))
		}
		else{
			visible()
		}
	...
	function handleScroll(){
		for(var i = 0; i < nodes.length; i++){
			var node = nodes[i]
			if(inView(node)){appear(node, invisibleClass, visibleClass)}
		}
	}
